...
ino(){ stat -c %i "$1" 2>/dev/null || stat -f %i "$1"; }   # GNU first: its stat -f prints filesystem stats
ino1=$(ino "$tmp/build/sub/a.yml"); inob1=$(ino "$tmp/build/sub/b.yml")
echo 'gone: 1' > "$tmp/build/sub/stale.yml"
( cd "$tmp" && bash "$OLDPWD/scripts/render.sh" >/dev/null )
ino2=$(ino "$tmp/build/sub/a.yml"); inob2=$(ino "$tmp/build/sub/b.yml")
[ "$ino1" = "$ino2" ] || { echo "FAIL: render replaced build/sub/a.yml instead of rewriting it in place (inode $ino1 -> $ino2)"; exit 1; }
[ "$inob1" = "$inob2" ] || { echo "FAIL: render replaced the copied build/sub/b.yml (inode $inob1 -> $inob2)"; exit 1; }
...
grep -A12 "job_name: cadvisor" core/prometheus/prometheus.yml.tpl | grep -q "regex: container_label_restartcount" || { echo "FAIL: cadvisor job does not drop container_label_restartcount"; exit 1; }
